	...
	class cuentaControlador extends mainModel{
		...
		public function actualizar_cuenta_controlador(){
			...
			if($user!="" && $password!=""){
				...
				if($login->rowCount()==0){
					$alerta=[
						...
						"Texto"=>"El nombre de usuario y la contraseña que acaba de ingresar no coincide con los datos de su cuenta",
						"Tipo"=>"error"
					];
				...
				$alerta=[
					"Alerta"=>"simple",
					"Titulo"=>"Ocurrió un error inesperado",
					"Texto"=>"Para actualizar los datos de esta cuenta debe de ingresar el nombre de usuario y contraseña, por favor ingrese los datos e intente nuevamente",
					"Tipo"=>"error"
				];
				return mainModel::sweet_alert($alerta);
				exit();
			...
			if($CuentaUsuario!=$DatosCuenta['CuentaUsuario']){
				$query3=mainModel::ejecutar_consulta_simple("SELECT CuentaUsuario FROM cuenta WHERE CuentaUsuario='$CuentaUsuario'");
			...
			if($CuentaEmail!=$DatosCuenta['CuentaEmail']){
				$query2=mainModel::ejecutar_consulta_simple("SELECT CuentaEmail FROM cuenta WHERE CuentaEmail='$CuentaEmail'");
